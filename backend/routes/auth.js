...
const { db }  = require('../db');
const { JWT_SECRET, REGISTER_SECRET, COOKIE_SECURE } = require('../config');
...
function sendAuth(res, token, username) {
...
router.post('/register', async (req, res) => {
  ...
  if (invite_code !== REGISTER_SECRET) {
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    sendAuth(res, token, user.username);
  } catch {
    res.status(500).json({ error: 'Ошибка сервера' });
  }
});
